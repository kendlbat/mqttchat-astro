<script lang="ts">
    ...
    let broker: string =
        window.location.hash?.slice(1).split(/(\||\%7C)/)[0] ||
        localStorage.getItem("mqtt-broker") ||
        "wss://test.mosquitto.org:8081";
    let anon: boolean =
        window.location.hash?.slice(1).split(/(\||\%7C)/)[1] == "1" ||
        localStorage.getItem("mqtt-password") == "";
    let user: string =
        window.location.hash?.slice(1).split(/(\||\%7C)/)[2] ||
        localStorage.getItem("mqtt-username") ||
        "";
    let password: string =
        window.location.hash?.slice(1).split(/(\||\%7C)/)[3] ||
        localStorage.getItem("mqtt-password") ||
        "";
    ...
    if (window.location.hash.split(/(\||\%7C)/).length > 1) {
        onSubmit();
    }
</script>
